-- Komplett Supabase setup för ny databas
-- Kör detta i din nya Supabase SQL Editor

-- ============================================
-- 1. GRUNDLÄGGANDE TABELLER (från supabase-complete-setup.sql)
-- ============================================
...
ALTER TABLE contract_clicks DROP CONSTRAINT IF EXISTS contract_clicks_plan_id_fkey;

-- Add foreign key constraint for contract_clicks after tables are created
-- Only add if both tables exist and have correct types
DO $$
BEGIN
  -- Check if electricity_plans table exists and has integer id
  IF EXISTS (
    SELECT 1 FROM information_schema.tables 
    WHERE table_name = 'electricity_plans' 
    ...
    BEGIN
      ...
      ADD CONSTRAINT contract_clicks_plan_id_fkey 
      FOREIGN KEY (plan_id) REFERENCES electricity_plans(id) ON DELETE SET NULL;
    EXCEPTION
      WHEN OTHERS THEN
...
CREATE INDEX IF NOT EXISTS idx_electricity_plans_type ON electricity_plans(type, active);
CREATE INDEX IF NOT EXISTS idx_electricity_plans_provider ON electricity_plans(provider);
CREATE INDEX IF NOT EXISTS idx_electricity_plans_recommended ON electricity_plans(recommended, active);

-- Tracking indexes
CREATE INDEX IF NOT EXISTS idx_contract_clicks_plan_id ON contract_clicks(plan_id);
CREATE INDEX IF NOT EXISTS idx_contract_clicks_created_at ON contract_clicks(created_at);
CREATE INDEX IF NOT EXISTS idx_share_tracking_session_id ON share_tracking(session_id);
...
-- Reminders indexes
CREATE INDEX IF NOT EXISTS idx_customer_reminders_email ON customer_reminders(email);
CREATE INDEX IF NOT EXISTS idx_customer_reminders_date ON customer_reminders(reminder_date);
CREATE INDEX IF NOT EXISTS idx_customer_reminders_sent ON customer_reminders(sent);
...
DROP TRIGGER IF EXISTS trigger_update_electricity_plans_updated_at ON electricity_plans;
...
COMMENT ON TABLE ai_knowledge IS 'AI knowledge base for chat assistance';
COMMENT ON TABLE ai_campaigns IS 'Active campaigns and offers';
COMMENT ON TABLE ai_providers IS 'Electricity providers information';
COMMENT ON TABLE electricity_plans IS 'Available electricity plans';
COMMENT ON TABLE contract_clicks IS 'Tracking for contract clicks';
COMMENT ON TABLE share_tracking IS 'Social media sharing tracking';
